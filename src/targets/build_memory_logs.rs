...
impl Framework for BuildMemoryLogsFramework {
    fn name(&self) -> &'static str { "build-memory-logs" }
    fn summary(&self) -> &'static str { "Build memory probe JSONL episode logs" }
...
fn list_log_files() -> Vec<String> {
    exec::read_dir(LOG_DIR).unwrap_or_default()
        .into_iter()
...
impl Variant for RotateByMonth {
    fn name(&self) -> &'static str { "rotate-by-month" }
    ...
    fn inspect(&self) -> Result<Inspection> {
        ...
        Ok(Inspection {
            ...
            notes: format!("{count} episode files ({:.1} total)", total_size as f64 / 1024.0 / 1024.0),
        })
    }
    fn apply(&self, dry_run: bool, _force: bool) -> Result<ApplyReport> {
        ...
        files.sort_by(|a, b| {
            std::fs::metadata(b).and_then(|m| m.modified()).ok()
...
impl Variant for PurgeOlder {
    fn name(&self) -> &'static str { "purge-older" }
    fn framework(&self) -> &'static dyn Framework { &FRAMEWORK }
    fn tier(&self) -> Tier { Tier::Safe }
    fn inspect(&self) -> Result<Inspection> {
        let files = list_log_files();
