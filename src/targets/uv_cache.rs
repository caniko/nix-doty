...
impl Framework for UvCacheFramework {
    fn name(&self) -> &'static str { "uv-cache" }
    fn summary(&self) -> &'static str { "Python UV package cache" }
    ...
    fn apply(&self, dry_run: bool, _force: bool) -> Result<ApplyReport> {
        if dry_run {
            ...
            return Ok(ApplyReport {
                ...
                errors: vec![format!("dry-run: would free {:.1} MiB", size as f64 / 1024.0 / 1024.0)],
            });
        }
        ...
        Ok(ApplyReport {
            framework: self.framework().name(),
            variant: self.name(),
            removed: 1, freed_bytes: 0, skipped: 0, errors: vec![],
        })
    }
}
...
impl Variant for UvCachePruneOlder {
    fn name(&self) -> &'static str { "prune-older" }
    fn framework(&self) -> &'static dyn Framework { &FRAMEWORK }
