...
impl Framework for CgroupResetFramework {
    fn name(&self) -> &'static str { "cgroup-reset" }
    fn summary(&self) -> &'static str { "Stuck cgroup scopes and D-state processes" }
...
impl Variant for StuckScopes {
    fn name(&self) -> &'static str { "stuck-scopes" }
    fn framework(&self) -> &'static dyn Framework { &FRAMEWORK }
    fn tier(&self) -> Tier { Tier::Confirm }
    fn inspect(&self) -> Result<Inspection> {
        let d_state = exec::run_stdout(&["ps", "-eo", "pid,stat,comm", "--no-headers"])
            .unwrap_or_default();
        let stuck_count = d_state.lines()
            .filter(|l| l.contains("D") || l.contains("T"))
            .count() as u64;

        let abandoned_scopes = exec::run_stdout(&["systemctl", "list-units", "--state=failed", "--no-legend", "--plain"])
            .unwrap_or_default()
            .lines()
        ...
        Ok(Inspection {
            ...
            notes: format!("{stuck_count} D/T-state processes, {abandoned_scopes} abandoned scopes"),
        })
    }
    fn apply(&self, dry_run: bool, force: bool) -> Result<ApplyReport> {
        ...
        let failed_scopes = exec::run_stdout(&["systemctl", "list-units", "--state=failed", "--no-legend", "--plain"])
            .unwrap_or_default();
