...
impl Framework for OpencodeCacheFramework {
    ...
    fn summary(&self) -> &'static str {
        "Opencode agent session cache: tool output and snapshots (disabled by default)"
    }
...
fn remove_subdir(parent: &str, sub: &str) -> Result<(u64, u64, Vec<String>)> {
    ...
    let entries = exec::read_dir(&path).unwrap_or_default();
    let bytes = exec::total_paths_size_bounded(&entries, 20_000).bytes;
...
impl Variant for PurgeToolOutput {
    ...
    fn inspect(&self) -> Result<Inspection> {
        ...
        let (total_count, total_bytes) = dirs
            .iter()
        ...
        Ok(Inspection {
            ...
            notes: format!(
                "opencode tool output cache across {count} users — disabled by default",
                count = dirs.len()
            ),
        })
    }
    fn apply(&self, apply: bool, _force: bool) -> Result<ApplyReport> {
        ...
        if !apply {
            ...
            return Ok(ApplyReport {
                ...
                skipped: count,
                errors: vec![format!(
                    "dry-run: report-only — would delete {count} tool output dirs ({})",
                    fmt_bytes(bytes)
                )],
            });
        }
...
impl Variant for PurgeSnapshots {
    ...
    fn inspect(&self) -> Result<Inspection> {
        ...
        Ok(Inspection {
            ...
            would_remove: total_count,
            notes: format!(
                "opencode state snapshots across {count} users — disabled by default",
                count = dirs.len()
            ),
        })
    }
    fn apply(&self, apply: bool, _force: bool) -> Result<ApplyReport> {
        ...
        if !apply {
            ...
            return Ok(ApplyReport {
                ...
                skipped: count,
                errors: vec![format!(
                    "dry-run: report-only — would delete {count} snapshot files ({})",
                    fmt_bytes(bytes)
                )],
            });
        }
...
impl Variant for PurgeLogs {
    ...
    fn inspect(&self) -> Result<Inspection> {
        ...
        Ok(Inspection {
            ...
            notes: format!(
                "opencode chat logs across {count} users — disabled by default",
                count = dirs.len()
            ),
        })
    }
    fn apply(&self, apply: bool, _force: bool) -> Result<ApplyReport> {
        ...
        if !apply {
            ...
            return Ok(ApplyReport {
                ...
                errors: vec![format!(
                    "dry-run: report-only — would delete opencode logs ({})",
                    fmt_bytes(bytes)
                )],
            });
        }
